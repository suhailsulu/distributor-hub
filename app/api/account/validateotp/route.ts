...
async function generateDBToken(
  ...
  while (true) {
    const rows =
      (await sql`SELECT id FROM password_reset_tokens WHERE token = ${token} LIMIT 1;`) as Array<{
        id: number;
      }>;
    if (rows.length === 0) {
      break;
  ...
  try {
    ...
    if (!email || !otp || !otp_type || !altcha) {
      return Response.json(
        { message: "email, otp, otp_type and altcha are required" },
        { status: 400 },
      );
    }
    ...
    const resetToken = await generateDBToken(
      sql as NeonQueryFunction<boolean, boolean>,
    );
    ...
    return Response.json(
      {
        message:
          "Password reset link sent to your email. Please follow the instructions in the email to reset your password. If you do not receive the email within a few minutes, please check your spam folder or try again.",
      },
      { status: 200 },
    );
  } catch (error) {
