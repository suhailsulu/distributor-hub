...
const Altcha = forwardRef<AltchaHandle, AltchaProps>(function Altcha(
  { onStateChange, expireMs = 15000 },
  ref,
) {
  const widgetRef = useRef<AltchaWidget & AltchaWidgetMethods & HTMLElement>(
    null,
  );
  const verifiedExpireTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const clearVerifiedTimer = useCallback(() => {
  ...
  const schedulePostVerifyExpiry = useCallback(() => {
    clearVerifiedTimer();
  ...
  }, [clearVerifiedTimer, schedulePostVerifyExpiry]);
