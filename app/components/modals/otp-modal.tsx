...
export function OtpModal({ isOpen, ...props }: OtpModalProps) {
  if (!isOpen) {
    return null;
  }

  return <OtpModalContent {...props} />;
}
...
function OtpModalContent({
  ...
  }, [disableClose, isSubmitting, onClose]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2
          id="otp-modal-title"
          className="pr-10 text-2xl font-bold text-[#1a2f4c]"
        >
          {title}
        </h2>
        ...
        <form onSubmit={submitOtp} className="mt-6 space-y-5" noValidate>
          ...
          <div className="space-y-1 text-sm text-[#253f5f]">
            <input
              ...
              {...register("altcha", {
                validate: (value) =>
                  !!value || "Please complete the CAPTCHA verification.",
              })}
            />
