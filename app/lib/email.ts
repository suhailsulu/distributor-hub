...
export async function sendForgotPasswordEmail(toEmail: string, link: string) {
  const template = createElement(ForgotPasswordEmailTemplate, {
    email: toEmail,
    link,
  });
  return await sendEmail(
    toEmail,
    "Reset your password (valid for 4 hours)",
    template,
  );
}

export async function sendRegistrationReceivedEmail(toEmail: string) {
  const template = createElement(RegistrationReceivedEmailTemplate, {
    email: toEmail,
  });
...
export async function sendUserBlockedEmail(toEmail: string) {
  const template = createElement(UserStatusChangeEmailTemplate, {
    email: toEmail,
    status: "blocked",
  });
...
export async function sendUserReactivatedEmail(toEmail: string) {
  ...
  return await sendEmail(
    toEmail,
    "Account status update: reactivated",
    template,
  );
}
...
export async function sendUserApprovedEmail(toEmail: string) {
  ...
  return await sendEmail(
    toEmail,
    "Application status update: approved",
    template,
  );
}
