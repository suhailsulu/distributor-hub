...
export function partitionUsers(users: ManagedUser[]) {
    const newUsers = users.filter((user) => NEW_USER_STATUSES.has(user.user_status));
    const existingUsers = users.filter((user) => EXISTING_USER_STATUSES.has(user.user_status));
