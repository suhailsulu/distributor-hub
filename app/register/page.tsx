...
const AltchaWidget = dynamic(
  () => import("../components/altcha/AltchaWrapper"),
  {
    ssr: false,
    loading: () => (
...
const inputClass = (hasError: boolean) =>
  `h-11 w-full rounded-lg border bg-white px-3 text-[#1b2f49] outline-none transition focus:ring-2 ${
    hasError
...
const textareaClass = (hasError: boolean) =>
  `min-h-24 w-full rounded-lg border bg-white px-3 py-2 text-[#1b2f49] outline-none transition focus:ring-2 ${
    hasError
      ? "border-red-400 focus:border-red-500 focus:ring-red-200"
      : "border-[#d6deea] focus:border-[#1377c5] focus:ring-[#1377c5]/20"
  }`;
...
export default function RegisterPage() {
  ...
  const submitOtpVerification = async (values: {
    otp: string;
    altcha: string;
  }) => {
    ...
    setSuccess(
      responseData?.message ||
        "Registration request submitted. We will review your application shortly.",
    );
    reset();
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#5aa3dd] px-4 py-8 sm:px-6 lg:px-8 flex items-center justify-center">
      ...
      <section className="relative mx-auto grid w-full max-w-5xl overflow-hidden rounded-3xl border border-white/40 bg-[#f6f8fc] shadow-[0_20px_40px_rgba(10,50,90,0.28)] md:grid-cols-[1fr_1.3fr]">
        <div className="flex min-h-[260px] flex-col justify-center gap-4 border-b border-[#cfddec] px-8 py-10 md:border-b-0 md:border-r md:border-r-[#cfddec] md:px-10 lg:px-12">
          <p className="text-[2rem] font-semibold leading-tight text-[#1a2f4c]">
            Register for Accessing
          </p>
          <div>
            <p className="text-4xl font-bold text-[#03589e]">
              Distributor Asset Hub
            </p>
          </div>
        ...
        <div className="px-6 py-8 sm:px-10 sm:py-10">
          ...
          >
            ...
            <Field label="Company *" error={errors.company?.message}>
              ...
              {isFetchingCompany && (
                <p className="mt-1 text-xs text-[#6f8aa8]">
                  Fetching company details from your email domain…
                </p>
              )}
              {companyLocked && !isFetchingCompany && (
                <p className="mt-1 text-xs text-[#3a7fc1]">
                  Company auto-filled from your email domain.
                </p>
              )}
            </Field>
              ...
              <textarea
                ...
                {...register("purpose", {
                  ...
                  minLength: {
                    value: 20,
                    message:
                      "Please provide a more detailed purpose (at least 20 characters).",
                  },
                  maxLength: {
                    value: 1000,
            ...
            <div className="space-y-1 text-sm text-[#253f5f]">
              ...
              <input
                ...
                {...register("altcha", {
                  validate: (v) =>
                    !!v || "Please complete the CAPTCHA verification.",
                })}
              />
              <AltchaWidget
                expireMs={15000}
                onStateChange={(ev) => {
                  if ("detail" in ev) {
                    const detail = (
                      ev as CustomEvent<{ payload?: string; state?: string }>
                    ).detail;

                    if (detail?.state === "verifying") {
                      clearErrors("altcha");
                      setValue("altcha", detail.payload ?? "", {
              ...
              {errors.altcha ? (
                <p className="text-xs text-red-600 mt-1">
                  {errors.altcha.message as string}
                </p>
              ) : null}
            </div>
            ...
            <p className="text-sm text-[#325377]">
              ...
              <Link
                href="/login"
                className="font-semibold text-[#0f75bd] hover:underline"
              >
                Sign in
              </Link>
            </p>
      ...
      <OtpModal
        ...
        content={
          <div className="space-y-3">
            <p>
              Registration request submitted. Enter the OTP sent to{" "}
              <strong>{pendingEmail}</strong> to verify your email address and
              complete the application process.
            </p>
            <div className="rounded-xl border border-[#d6deea] bg-white/70 p-3 text-sm text-[#325377]">
              <p className="font-semibold text-[#1a2f4c]">
